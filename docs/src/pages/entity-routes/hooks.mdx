...
You can pass a [`HookSchema`](/api-reference/globals#hookschema) in the `hooks` key of the options (EntityRouteOptions docref) argument,
available when making entity routers (using [`makeEntityRouters`](/api-reference/globals#makeentityrouters)) or directly in the `@EntityRouter` (docref).
