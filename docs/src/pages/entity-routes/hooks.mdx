...
You can pass a [`HookSchema`](/definitions#hookschema) in the `hooks` key of the options (EntityRouteOptions docref) argument,
available when making entity routers (using [`makeEntityRouters`](/definitions#makeentityrouters)) or directly in the `@EntityRouter` (docref).
