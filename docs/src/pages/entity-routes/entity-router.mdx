...
<DocAlert>
    The name distinction is to avoid any conflict between the service ([`EntityRouter`](/definitions/classes/entityrouter)) and the decorator
    (`EntityRoute`).
</DocAlert>
...
Koa-like with `(ctx, next) => void`), you can make your own [`maker`](/entity-routes/introduction#maker/) with your own custom [`ContextAdapter`](/definitions#contextadapter). TODO link
