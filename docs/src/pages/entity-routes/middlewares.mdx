...
[`RequestContext`](/definitions/definitions#requestcontext).
...
[`EntityRouteOptions`](/definitions/definitions#entityrouteoptions), which means that you can both register a local
middleware for a specific [`EntityRouter`](/definitions/classes/entityrouter) or register global middlewares by passing
them to the [`makeEntityRouters`](/definitions/definitions#makeentityrouters).

The implementation itself of a middleware will depend of the [`maker`](/entity-routes/introduction#maker/) used.

-   If you use Koa, you can use the [`makeKoaEntityRouters`](/definitions/definitions#makekoaentityrouters)
