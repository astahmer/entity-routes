...
maxDepth is independant from each other.

---

registerSubresource(User, Article, "articles", { maxDepth: 5 }); registerSubresource(Article, Comment, "comments");
registerSubresource(Comment, Upvote, "upvotes", { maxDepth: 3 }); That means you could have a `maxDepth = 5` on
`User.articles` subresource but still no
