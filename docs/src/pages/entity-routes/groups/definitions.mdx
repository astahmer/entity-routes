...
> relations. A relation IRI is made with the [`idToIRI`](/definitions#idtoiri) function using the (route path|entity table name) & id.

You can opt-out of using IRIs by setting the `useIris` key of [`EntityRouteOptions`](/definitions#entityrouteoptions) to `false`.
