...
`EntityRouter` set up on the `Article` entity with `["create", "update", "list"]` **operations**, you would have those
routes generated :
...
There is a `pretty` query parameter (?pretty=true) available to make it less verbose, it looks kind of like a GraphQL
request body, as you can see below.
