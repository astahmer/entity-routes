...
By using the [@Groups](/definitions#groups) decorator, you can expose the decorated property through one or more [`route scopes`](/entity-routes/route-scope/).
...
There are also the [shortcuts](operations#shortcuts) `all` and `basic` that you can use directly, or use as a route scope value.
...
Let's say we want to expose the user name through basic [`CRUD_OPERATIONS`](/definitions#const-crud_operations) for any context.
...
To expose a nested property, you need to decorate with [`@Groups`](/definitions#groups) both the relation and the property with the same operation.
