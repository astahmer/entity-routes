...
"Exposed" has different meaning for the `persist` [**operations**](/entity-routes/operations/) (_create/update_) and the
`read` [**operations**](/entity-routes/operations/) (_details/list_).
...
### Basic [**operations**](/entity-routes/operations/)
...
To expose a nested property, you need to decorate with [`@Groups`](/definitions#groups) both the relation and the
property with the same [**operation**](/entity-routes/operations/).
...
That's it ! The user details route will contain a role object with a title property inside. This works the exact same
way with a property that is a collection
