...
You can expose a property locally (for an entity [`route scope`](/route-scope/)) using an object with entity [`route scope`](/route-scope/) as keys and values as operations :
...
Or you can expose a property globally (for **any** entity [`route scope`](/route-scope/)) using an array of operations :
...
But we would also like to expose it for any [`route scope`](/route-scope/), on a custom operation _(which is matching a custom route action with the same operation)_.
...
To expose a nested property, you need to decorate with `@Groups` both the relation and the property with the same operation.
