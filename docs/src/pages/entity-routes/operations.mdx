...
There are two kinds of operations : [`RouteOperation`](/api-reference/globals#routeoperation) and [`GroupsOperation`](/api-reference/globals#groupsoperation).
...
-   "basic" means every basic [`CRUD_OPERATIONS`](/api-reference/globals#const-crud_operations) : `[create, list, details, update]`

<DocAlert status="info">

Each [`GroupsOperation`](globals#groupsoperation) match one from [`RouteOperation`](/api-reference/globals#routeoperation), except `delete` since there is no property exposed in a DELETE route response.
