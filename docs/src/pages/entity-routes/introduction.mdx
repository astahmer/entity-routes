---
title: Introduction
description: An introduction to entity-routes features, its perks and the problem it solves.
---
...
-   [`Subresources`](/entity-routes/subresources/introduction): Entity properties that have dedicated endpoints, can be
    nested
...
-   Built-in powerful [`filters`](/filters/introduction) for lists (`Search`/`Pagination`) that should fit well for 99%
    use cases
...
-   Inferred entity route mappings (with generated endpoints to get the summary of an entity exposed properties for each
    contexts)
-   [Computed properties](/groups/computed-property): methods, possibly async, exposed as properties
...
-   Standardized REST [responses](/entity-routes/introduction)
